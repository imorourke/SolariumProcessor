...
%\usepackage[pdftex, pdfauthor={Ian O'Rourke}, pdftitle={Jib ISA}]
\pdfinfo{
	/Author (Ian O'Rourke)
	/Title (Jib Instruction Set Architecture)
	/CreationDate (\today)
	/Subject (Jib ISA)
	/Keywords (Jib;CBuoy;C/Buoy;ISA;Processor;Computer)
}
...
\begin{table}[h!]
    ...
    \begin{tabular}{c|cc}
        ...
        \hline
    \end{tabular}
    \caption{Registers R0 through R5 are reserved for system-level parameters. R6 through R8 are reserved for C/Buoy functionality, by convention. All remaining registers are general-purpose.}
    \label{table:register-setup}
\end{table}

The program counter indicates the next instruction to be read. At the beginning of a processor cycle, the instruction at the memory address of the program counter is read in and processed. The program counter is incremented at the end of the cycle. It is recommended to use the \texttt{jmp} or \texttt{jmpr} instructions (see Section \ref{sec:instructions}) to modify the program counter, as opposed to writing to the register directly.
...
This is contrasted with the typical assembly language formatting, which is provided as follows, where the number of arguments depends on the required number of arguments for the instruction.
...
\begin{table}[h!]
	...
	\begin{tabular}{c|cc|c|c|cc|c|}
		\hline
		Type & Code & Size in Bytes & Description & Type & Code & Size in Bytes & Description \\
		\hline
		 U8 & 1 & 1 & Unsigned byte & I8 & 2 & 1 & Signed byte \\
		 U16 & 3 & 2 & Unsigned short& I16 & 4 & 2 & Signed short \\
		 U32 & 5 & 4 & Unsigned integer & I32 & 6 & 4 & Signed integer \\
...
\begin{table}[h!]
	...
	\begin{footnotesize}
		\begin{tabular}{c|cc|l|l}
			...
			C & 0 & A & \texttt{jmp [a]} & \texttt{\$PC = R[a]} \\
			C & 0 & B & \texttt{jmpr [a]} & \texttt{\$PC += R[a]} \\
			B & 0 & C & \texttt{jmpri <imm>} & \texttt{\$PC += Imm} (Signed) \\
			A & 0 & F & \texttt{halt} & Stop Program Execution \\
			...
			D & 1 & 4 & \texttt{ldn [a]} & \texttt{R[a] = mem[\$PC + 4]}, \texttt{\$PC += 8} \\
			D & 1 & 5 & \texttt{ldno [a]} & \texttt{R[a] = mem[\$LDO + PC + 4]}, \texttt{\$PC += 8} \\
			G & 1 & 6 & \texttt{sav [a] [b]} & \texttt{mem[R[a]] = R[b]} \\
			G & 1 & 7 & \texttt{savr [a] [b]} & \texttt{mem[\$PC + R[a]] = R[b]} \\
			F & 1 & 8 & \texttt{copy [a] [b]} & \texttt{R[a] = R[b]} \\
			H & 1 & 9 & \texttt{conv [a] [b]} & \texttt{R[a] = R[b]} \\
			...
			A & 4 & 0 & \texttt{inton} & Turn Interrupts On, set \texttt{\$STAT} \\
			A & 4 & 1 & \texttt{intoff} & Turn Interrupts Off, set \texttt{\$STAT} \\

			A & 5 & 0 & \texttt{brk} & Debug Breakpoint (capturable by hardware if supported; \\
			{} & {} & {} & {} & otherwise, acts as a \texttt{noop}) \\

			I & A & 0 & \texttt{add [dst] [a] [b]} & \texttt{R[dst] = R[a] + R[b]} \\
			I & A & 1 & \texttt{sub [dst] [a] [b]} & \texttt{R[dst] = R[a] - R[b]} \\
			I & A & 2 & \texttt{mul [dst] [a] [b]} & \texttt{R[dst] = R[a] * R[b]} \\
			I & A & 3 & \texttt{div [dst] [a] [b]} & \texttt{R[dst] = R[a] / R[b]} \\
			I & A & 4 & \texttt{rem [dst] [a] [b]} & \texttt{R[dst] = R[a] \% R[b]} \\
			G & A & 5 & \texttt{neg [dst] [a]} & \texttt{R[dst] = -R[a]} \\

			I & B & 0 & \texttt{band [dst] [a] [b]} & \texttt{R[dst] = R[a] \& R[b]} \\
			I & B & 1 & \texttt{bor [dst] [a] [b]} & \texttt{R[dst] = R[a] | R[b]} \\
			I & B & 2 & \texttt{bxor [dst] [a] [b]} & \texttt{R[dst] = R[a] $\wedge$ R[b]} \\
			I & B & 3 & \texttt{bshl [dst] [a] [b]} & \texttt{R[dst] = R[a] << R[b]} \\
			I & B & 4 & \texttt{bshr [dst] [a] [b]} & \texttt{R[dst] = R[a] >> R[b]} \\
			G & B & 5 & \texttt{bnot [dst] [a]} & \texttt{R[dst] = \textasciitilde R[a]} \\
			D & B & 6 & \texttt{bswp [dst] [a]} & \texttt{R[dst] = R[a] (Swapped Bytes)} \\
			\hline
		\end{tabular}
	\end{footnotesize}
...
\begin{table}[h!]
    ...
    \begin{tabular}{l|l}
        ...
        \texttt{.text "[TEXT]"} & Loads the text into memory, starting at the current memory location, \\
        ...
        \texttt{.reserve [num]} & Reserves the the next \texttt{[num]} bytes of memory for data, \\
        & filling the memory with zeros \\
        \hline
        \texttt{.u8/.u16/.u32 [num]} & Loads the given unsigned integer at the current location \\
        \texttt{.i8/.i16/.ui32 [num]} & Loads the given signed integer at the current location \\
        \texttt{.f32 [num]} & Loads the given floating point number at the current location \\
        \hline
    \end{tabular}
...
\begin{table}[h!]
    ...
    \begin{tabular}{cc|cc|cc|cc}
        ...
        09 & \charmapescape{t} Tab & 29 & \charmap{)} & 49 & \charmap{I} & 69 & \charmap{i} \\
...
Devices are memory-mapped in Jib. This means that reading or writing to special regions in memory facilitate the communication with these external devices. In a typical Jib computer, the device region consists of up to 64 devices, starting at memory address \texttt{0xFFFFA000}, with each device allocating up to 32 \textit{(1024?)} bytes of memory. Not all devices will make use all the available memory slots for a given device. In these cases, a memory exception will be provided if any of these invalid addresses are read from or written to. Available devices are listed in Table \ref{table:dev-ids}.
...
\begin{table}[h!]
	...
	\begin{tabular}{l|ll}
		...
		\hline
		0 & None & {} \\
		1 & Serial I/O & (Section \ref{sec:dev-serial}) \\
		2 & IRQ Clock & (Section \ref{sec:dev-irq-clock}) \\
		3 & RTC Clock & (Section \ref{sec:dev-rtc-clock}) \\
		4 & RTC Timer & (Section \ref{sec:dev-rtc-timer}) \\
		5 & Block Device & (Section \ref{sec:dev-block-device}) \\
		\hline
	\end{tabular}
	\caption{Device IDs allow for easy enumeration of different devices}
	\label{table:dev-ids}
\end{table}

\subsection{Serial Input and Output}
\label{sec:dev-serial}
...
\begin{table}[h!]
    ...
    \begin{tabular}{l|lll}
        ...
        \texttt{8} & u8 & Read/Write & If non-zero, interrupt to write to when input is received \\
        \hline
    \end{tabular}
...
\begin{table}[h!]
	...
	\begin{tabular}{l|lll}
		...
		\texttt{10} & u32 & Read/Write & The hardware IRQ to trigger, if non-zero. \\
		\hline
	\end{tabular}
	\caption{Serial Input and Output device provides a simple data structure to read and write data streams}
...
\label{sec:dev-rtc-clock}

The RTC clock device provides a clock providing accurate wall-time. This provides direct memory access to the current time, and is typically provided as UTC time. The device specification is provided in Table \ref{table:dev-rtc-clock}.
...
\begin{table}[h!]
	...
	\begin{tabular}{l|lll}
		...
		\hline
		\texttt{0} & u16 & Read & Device ID \\
		\texttt{2} & u16 & Read & Years \\
		\texttt{4} & u8 & Read & Months (1-12) \\
		\texttt{5} & u8 & Read & Days (1-31) \\
		\texttt{6} & u8 & Read & Hours (0-23) \\
		\texttt{7} & u8 & Read & Minutes (0-59) \\
		\texttt{8} & u8 & Read & Seconds (0-59) \\
		\texttt{9} & u8 & Read & Hundredths of a second (0-99) \\
		\hline
	\end{tabular}
	\caption{Real time clock provides current wall time for programs}
	\label{table:dev-rtc-clock}
\end{table}

\subsection{RTC Timer}
\label{sec:dev-rtc-timer}

The RTC Timer device allows for timing parameters based on a specific wall time, as opposed to a number of CPU cycles, as provided by the IRQ clock in Section \ref{sec:dev-irq-clock}. The specification for the RTC timer is provided in Table \ref{table:dev-rtc-timer}.
...
\begin{table}[h!]
	...
	\begin{tabular}{l|lll}
		...
		\hline
		\texttt{0} & u16 & Read & Device ID \\
		\texttt{2} & u8 & Read/Write & Selected IRQ \\
		\texttt{3} & u8 & Read/Write & Timer repeat flag \\
		\texttt{4} & u32 & Read/Write & Timer period, in milliseconds \\
		\hline
	\end{tabular}
	\caption{Real time timer provides a way to provide timing based on elapsed time instead of cycles}
	\label{table:dev-rtc-timer}
\end{table}

\subsection{Block Device}
\label{sec:dev-block-device}

A block device allows for reading and writing from a mass storage device. This provides a way to read a block device in chunks, as a "block" of data. Block device mappings are provided in Table \ref{table:dev-block-device}.
...
\begin{table}[h!]
	...
	\begin{tabular}{l|lll}
		...
		\hline
		\texttt{0} & u16 & Read & Device ID \\
		\texttt{2} & u8 & Read/Write & Drive parked  \\
		\texttt{3} & u8 & Read & Drive ready? \\
		\texttt{4} & u32 & Read/Write & Drive Target Offset \\
		\texttt{8} & u8 & Write & Move to offset when non-zero \\
		\texttt{9} & u8 & Write & Write data when non-zero \\
		\texttt{10} & u16 & Read/Write & Memory window size (max of 256) \\
		\texttt{12} & [256]u8 & Read/Write & Memory window (if 0 sets to max) \\
		\hline
	\end{tabular}
	\caption{Block device allows reading and writing of data from a mass-storage device}
	\label{table:dev-block-device}
\end{table}
...
\lstinputlisting[caption={Limited counter program}, label={listing:example-counter}]{../jib/jib-asm/examples/counter.jsm}
...
\lstinputlisting[caption={Infinite counter program with register reference names}, label={listing:example-infinite-counter}]{../jib/jib-asm/examples/infinite_counter.jsm}

\subsection{Hello World}

Listing  \ref{listing:hello-world} provides a program that will infinitely write "hello, world" to the serial device, expected to be memory-mapped into the Jib at \texttt{0xFFFFA000}. This makes use of the \texttt{.loadtext} command, which loads the text, and an additional null-terminator at the end, directly into memory. It also makes use of a print-string function call to handle printing string values to the serial output devices.

\lstinputlisting[caption={Infinite "hello, world!" program}, label={listing:hello-world}]{../jib/jib-asm/examples/hello_world.jsm}

\pagebreak

\subsection{Serial Echo}

Listing \ref{listing:serial-echo} provides a serial echo program, which echos any text provided by the default serial input device back out via the serial echo device. This serial device is expected to be provided at the starting location \texttt{0xFFFFA000}.

\lstinputlisting[caption={Serial echo program reads in text characters and immediately outputs via the output device}, label={listing:serial-echo}]{../jib/jib-asm/examples/serial_echo.jsm}
...
\begin{tabular}{rl}
    ...
    & fnint \texttt{FunctionName}(\texttt{Variable}[, \texttt{Variable}\dots]) \{ \textlangle StatementList\textrangle \} \\
    ...
    & asm \{ [\textlangle WordLiteral; \textrangle \dots] \} \\
    ...
    & (\textlangle BaseExpression\textrangle) \\
    & \textlangle Expression \textrangle[\textlangle Expression \textrangle] \\
    BinaryOp & $\rightarrow$  \\
...
This makes use of a few extra conventions. At the start of each function, stack space is reserved both for any local variables, as well as any temporary variables, that are required. A few extra registers are defined in the Table \ref{table:register-setup}.
...
A simple preprocessor allows for mixing multiple files into one source unit. This follows a similar syntax to the C preprocessor, albeit much simpler.

\begin{itemize}
	\item \texttt{\#include <file>} includes the provided file into the current source.
	\item \texttt{\#ifdef <name>} and \texttt{\#ifndef <name>} only includes the subsequent lines if the name is defined or not defined. Must have a matching \texttt{\#endif}.
	\item \texttt{\#else} is coupled with an \texttt{\#if*} statement to include if the original statement is false. Only one may be provided per \texttt{\#if*} statement.
	\item \texttt{\#endif} ends an if statement.
	\item \texttt{\#define <name>} marks the provided name as defined.
	\item \texttt{\#undef <name>} marks the provided name as undefined.
\end{itemize}
...
\lstinputlisting[label={listing:example-cbuoy},caption={Example C/Buoy program for printing out a string to a serial output device}]{../cbuoy/cblang/examples/printing.cb}
...
\subsection{Compiler Mode}

Compiling can take place in one of two modes. The first is in a kernel mode, in which a specific stack location and program starting location is provided. The second is application mode, which compiles applications in a position-independent location.

\subsubsection{Kernel Mode}

Programs compiled in kernel mode should be loaded into memory at the specific location, as items are referred to by specific memory addresses and not by a relative address. This allows for the best performance at runtime and is helpful for the initial stack location of the lowest-level of software. It also makes it easy to provide a "base" program, which will use a distinct and defined stack, and is useful for the lowest-level of software running on the processor.

When compiling in kernel mode, functions, types, constants, and globals may be exported as a sort of header file that can be used in application-mode applications. Any supported type that does not start with an underscore will be exported and made available for use in the generated header file. This allows for sharing of common library functions between different applications more easily.

\subsubsection{Application Mode}

Programs compiled in application mode require additional information while loading, as they are position-independent. A program compiled in application mode must have the program base address provided in the \texttt{Load Offset} register (Section \ref{sec:registers}), as address references are made relative to that location. This allows a kernel to load multiple applications in different positions at runtime, allowing for more complicated program interactions, such as simultaneous application running/threading, by the base operating system, transparent to the running application.

\subsection{Assembly Integration}
...
\begin{table}[h!]
    ...
    \begin{tabular}{l|cl}
        ...
        \texttt{sizeof} & \texttt{\#\{IDENTIFIER\}\#} & Provides the \texttt{sizeof} for the provided \\ & & type or symbol \\
        \hline
    \end{tabular}
...
\lstinputlisting[label={listing:example-cbuoy-print-asm},caption={Replacement print function to improve the speed of the resulting output}]{../cbos/kernel/kserialio.cb}
...
Through direct modification of the stack, we can create a set of utilities to swap between different threads at runtime. Listing \ref{listing:cbuoy-threading-utils} provides several functions. \texttt{k\_tsk\_init} provides means to register a new thread. \texttt{k\_tsk\_main} is a function that is called to transition from the main thread into the inner executing threads. \texttt{k\_tsk\_yield} is both an interrupt handler (for a periodic device like a timer) and also a function that may be called by threaded functions to yield their execution to the next thread. This is simple round-robin threading, so there is no scheduler implemented in this example.

\lstinputlisting[caption={Utilities that use a mix of Jib assembly and C/Buoy provide mechanisms to create threads at runtime and switch between threads, either through preemptive interrupts or task yielding.}, label={listing:cbuoy-threading-utils}]{../cbos/kernel/ktsk.cb}

\pagebreak

\section{Filesystem}

The CBFS filesystem provides basic I/O capabilities for the Jib computer. It is based on a file allocation table and provides 16-bit integer identification for file sectors. A fuse driver exists for usage of the filesystem on POSIX systems.

\subsection{Volume Header}

The basic filesystem consists of the following for a CBFS volume, provided in Table \ref{table:cbfs-volume-header}.

\begin{table}[h!]
    \centering
    \begin{tabular}{l|cc|l}
        \hline
        Sector & Sector Byte Offset & Type & Parameter \\
        \hline
        0 & 0 & \texttt{u16} & CBFS Version (Currently only \texttt{1}) \\
        0 & 2 & \texttt{u16} & Sector Bytes Size \\
        0 & 4 & \texttt{u16} & Number of Sectors \\
        0 & 6 & \texttt{u16} & Root sector ID \\
        0 & 8 & \texttt{u32} & Volume Flags \\
        0 & 12 & \texttt{u8\charmap{[}32\charmap{]}} & Volume Name \\
        1 to $\lceil 2N / S \rceil$ & 0 & \texttt{u16\charmap{[}N\charmap{]}} & Sector Table \\
        \hline
    \end{tabular}
    \caption{Format for the volume header, where \texttt{S} is the sector byte size, and \texttt{N} is the number of bytes in each sector.}
    \label{table:cbfs-volume-header}
\end{table}

The total volume size, in bytes, is given by the product of the Sector Byte Size, $S$, by the Number of Sectors, $N$. The first sector is always taken up by volume header. The next set of sectors is taken up by the sector status. This will take up $\lceil 2 \cdot N / S \rceil$ sectors. This allows the first root sector ID, allowed for data, to be $1 + \lceil 2 \cdot N / S \rceil$. All sectors from the root to $N$ are valid entries for user data. The global base address in bytes for any sector $I$ can be obtained by $I \cdot S$.

Sector IDs within the sector table may be identified in the following ways. The first is with \texttt{0x0000}, which is a null node. A node with a value of null is unused, while any other value signifies that there is data within the node. If a node is used, the ID value points to the node containing the sector with the next set of data. If there is no next set of data the node is marked with the special node ID \texttt{0xFFFF}, signifying that there is no subsequent node and the current ID represents the final sector of data. This may look like the following in a simplified node table, provided in Table \ref{table:node-table-example}.

\begin{table}[h!]
    \centering
    \begin{tabular}{c|c|l}
        \hline
        Node ID & Sector Status & Description \\
        \hline
        \texttt{0} & \texttt{0x0000} & Unused Node \\
        \texttt{1} & \texttt{0x0003} & Node \texttt{2} Sector 2 \\
        \texttt{2} & \texttt{0x0001} & Node \texttt{2} Sector 1 (First) \\
        \texttt{3} & \texttt{0x0005} & Node \texttt{2} Sector 3 \\
        \texttt{4} & \texttt{0xFFFF} & Node \texttt{4} First and Final Sector \\
        \texttt{5} & \texttt{0xFFFF} & Node \texttt{2} Sector 4 (Final) \\
        \hline
    \end{tabular}
    \caption{Example node table for six nodes, with two root nodes provided as nodes \texttt{2} and \texttt{4}}
    \label{table:node-table-example}
\end{table}

\subsection{Filesystem Entries}

A filesystem entry may either be a file or a directory entry. The base entry for any directory or file consists of an entry header. This is defined in Table \ref{table:cbfs-entry-header}. Entries are always identified by the root sector ID, as this is the sector that contains the entry information.
...
\begin{table}[h!]
    ...
    \begin{tabular}{cc|l}
        ...
        \hline
        0 & \texttt{EntryType} & Entry Type \\
        2 & \texttt{u16} & Parent Directory Entry ID \\
        4 & \texttt{DateTime} & Modification Time \\
        8 & \texttt{u32} & Payload Size \\
        \hline
    \end{tabular}
    \caption{Entry header information provides the basics for the entry type, parent directory, and other helpful metadata for referencing entry parameters.}
    \label{table:cbfs-entry-header}
\end{table}

After the entry header, data is provided for each of the entry types. For a file entry, this data is just the file contents. For a directory, this is a list of directory entry information. One entry is present for each file or directory contained within the entry. The directory entry information is provided in Table \ref{table:cbfs-directory-entry}. Notice that the entry type is provided in both the entry header and the directory information. This small amount of data duplication is designed for each file enumeration; however, ensure that these two parameters remain consistent for a given filesystem entry.
...
\begin{table}[h!]
    ...
    \begin{tabular}{cc|l}
        ...
        Byte Offset & Type & Description \\
        \hline
        0 & \texttt{u16} & Base Block / Data Entry ID \\
        2 & \texttt{u8} & Attributes \\
        3 & \texttt{EntryType} & Entry Type \\
        8 & \texttt{u8\charmap{[}60\charmap{]}} & File Name \\
        \hline
    \end{tabular}
    \caption{Directory entries provide metadata for the file entry and allow quick enumeration for filesystem contents.}
    \label{table:cbfs-directory-entry}
\end{table}

The payload size for a directory entry should always be a multiple of the directory entry size, \texttt{64}, as there should be one and only one directory entry per contained entry in the filesystem. If an entry is in the filesystem but not in a directory entry, it is orphaned and the filesystem is in an inconsistent state. Likewise, if any Entry ID is referenced by two or more directory entries, the filesystem is in an inconsistent state.

Note that the root entry, as specified in the volume header, must always be a directory for the filesystem to be valid.

\subsection{Additional Types}

\subsubsection{EntryType}

The \texttt{EntryType} is an enumeration of size \texttt{u8}, where a value of \texttt{1} represents a directory entry, while a value of \texttt{2} is a file entry. Any other value is not valid.

\subsubsection{Date}

The \texttt{Date} type represents a calendar date entry.
...
The \texttt{Time} type represents a calendar time entry.
...
\begin{table}[h!]
    ...
    \begin{tabular}{cc|l}
        ...
        1 & \texttt{u8} & Minute \\
        2 & \texttt{u8} & Second \\
        3 & \texttt{u8} & Hundredths of a Second \\
        \hline
    \end{tabular}
\end{table}
...
The \texttt{DateTime} type combines both a date and a time together into a single type to provide a precise time instance.
\begin{table}[h!]
    \centering
    \begin{tabular}{cc|l}
        \hline
